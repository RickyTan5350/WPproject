...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $status = $_POST['status'];
    $manager_id = $_SESSION['user_id']; // assuming you store manager's user_id in session
    ...
    $stmt = $conn->prepare("INSERT INTO audit_log (booking_id, manager_id, action) VALUES (?, ?, ?)");
    $stmt->bind_param("iis", $booking_id, $manager_id, $status);
    $stmt->execute();
    $stmt->close();
}
// ...existing code...
